...
for i in range(magnified_width):
    horizon_blue_func[i] = pchip(np.arange(0, magnified_height), img[:, i, 0])

horizon_green_func = np.zeros(magnified_width, dtype=object)
for i in range(magnified_width):
    horizon_green_func[i] = pchip(np.arange(0, magnified_height), img[:, i, 1])

horizon_red_func = np.zeros(magnified_width, dtype=object)
for i in range(magnified_width):
    horizon_red_func[i] = pchip(np.arange(0, magnified_height), img[:, i, 2])
